...
Gunicorn runs with `--workers 2 --threads 4 --worker-class gthread`, not the bare default (a single sync worker). With only one worker, every request — including the Wagtail admin's own autosave (fires on a 500ms debounce while editing) and its live-preview re-render — queued up behind whatever the process was already doing, which is what made admin editing feel slow. `gthread` was chosen over more sync workers because its threads share memory within a process, which matters on the small (2 vCPU / ~2GB RAM) VM this is deployed on. Note this doesn't fix the underlying cause of any *single* request being slow: no `TASKS` backend is configured, so Wagtail's per-save reference-index and search-index updates (`django_tasks`'s `ImmediateBackend`) still run synchronously inside the request instead of in the background. Configuring a real backend (e.g. `django_tasks`'s database backend) would fix that too, but needs a permanently-running worker process (`manage.py db_worker`) — a real memory cost on this VM, not done here.
